...
- 2026-07-27 : reste sur l'upstream `luukvbaal/statuscol.nvim`. Le fork `NeOzay/statuscol.nvim` a été inspecté lors de la migration vers `plugins/` : son HEAD est un **ancêtre strict** de la version installée (aucun commit propre, 4 commits de retard). Le câbler aurait été une régression — voir [`statuscol-design.md`](statuscol-design.md) §10.
- 2026-07-28 : fix foldcolumn absente au démarrage pour certains buffers. `cond.setup_win_autocmd()` (FileType/WinEnter/BufWinEnter) n'est enregistré que dans le `config()` de statuscol, donc **après** que lazy.nvim ait ouvert les buffers/fenêtres initiaux (args multiples, session `persistence.nvim`, splits déjà en place) — leurs événements avaient déjà été émis, `apply_win_options` n'était donc jamais appelé sur eux. L'ancien rattrapage ne visait que le buffer courant (`nvim_exec_autocmds("BufWinEnter", { buf = ... })`). Remplacé par une boucle sur **toutes** les fenêtres de tous les tabpages (`cond.apply_win_options(win, buf)` directement) au démarrage.
