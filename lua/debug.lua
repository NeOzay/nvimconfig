...
-- Chargé via dofile() dans init.lua pour éviter le conflit avec le module natif Lua `debug`
...
cmd("AutocmdTrace", function(args)
	...
	elseif args.args == "off" then
		...
		vim.notify("Autocmd tracing " .. (vim.g.autocmd_trace and "ON → " .. log_path or "OFF"), vim.log.levels.INFO)
	end
end, {
	nargs = "?",
